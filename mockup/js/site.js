...
const NAMES_EN={kraft:'Kraft and caps',menta:'Mint and tangerine',ceu:'Sky and navy',pessego:'Peach and blue',limao:'Lemon and graphite',rosa:'Pink and green',noite:'Midnight and caps',oceano:'Ocean and sand',alperce:'Apricot and plum',algodao:'Candyfloss',reciclado:'Recycled paper and orange',festa:'Bottle-cap party',agua:'Aqua and coral'};
...
function setFavicon(P,dark){
  try{
    const act=P.act.length===4?'#'+[...P.act.slice(1)].map(c=>c+c).join(''):P.act;
...
function applyPal(id){
  ...
  const vars={'--cover':P.cover,'--cover-deep':mix(P.cover,'#000',dark?70:80),'--on-cover':P.on,'--on-cover-soft':mix(P.on,P.cover,86),'--cover-ink':mix(P.on,P.cover,86),
   ...
   '--ring':mix(P.pen,'transparent',26),'--tape':'rgba(255,255,255,.6)','--action':P.act,'--action-ink':P.ai,'--logo-1':P.act,'--logo-2':mix(P.act,'#000',72),'--logo-3':dark?mix(P.act,'#000',40):P.pen,'--cap1':P.caps[0],'--cap2':P.caps[1],'--cap3':P.caps[2],'--cap4':P.caps[3]};
